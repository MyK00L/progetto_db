...
CREATE TABLE IF NOT EXISTS PdPStazione(ID SERIAL PRIMARY KEY, IDPdP INTEGER UNIQUE NOT NULL, Binario TEXT NOT NULL,
    CONSTRAINT fk_pdp FOREIGN KEY (IDPdP) REFERENCES PuntoDiPassaggio(ID));
DO
$$
    BEGIN
...
END
$$;
CREATE TABLE IF NOT EXISTS Turno(IDPersona INTEGER NOT NULL, IDTreno INTEGER NOT NULL, Data DATE NOT NULL);
CREATE TABLE IF NOT EXISTS Persona(ID SERIAL PRIMARY KEY, Nome TEXT, Cognome TEXT, Ruolo TipoRuolo);
...
CREATE TABLE IF NOT EXISTS Esercizio(IDConvoglio INTEGER NOT NULL, IDTreno INTEGER NOT NULL, IDLocomotiva VARCHAR(12), Data DATE NOT NULL);
...
CREATE OR REPLACE VIEW RitardoPdP AS SELECT at.OrarioArrivo, at.OrarioPartenza, a.DataArrivo, a.DataPartenza, categoria, numero, at.idpdp, (CURRENT_DATE + COALESCE(OrarioPartenza, OrarioArrivo)) AS orario, COALESCE(datapartenza, dataarrivo) as data,
    ...
    FROM treno
        ...
        LEFT OUTER JOIN (SELECT * FROM attraversamento WHERE COALESCE(datapartenza, dataarrivo)::date = now()::date) a on treno.numero = a.idtreno  AND a.idtreno = at.idtreno AND a.idpdp = pdp.id AND pdpa.id = pdp.idastratto;
